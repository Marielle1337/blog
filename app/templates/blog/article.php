...
<!--  Ajouter des commentaires -->
    ...
    <fieldset> <legend>Ajouter un commentaire</legend>
        ...
        <label for="auteur">Votre nom
            <input id="auteur" type="text" name="author"  placeholder="Votre nom" <?php if(isset($_POST['author'])) { echo 'value="'.$_POST['author'].'"';}?> >
            <?php if (isset($errors['author'])) { echo $errors['author'] ;} ?>
        </label>

        <label for="email">Email
            <input id="email" type="mail" name="email"  placeholder="Votre email (facultatif)" <?php if(isset($_POST['email'])) { echo 'value="'.$_POST['email'].'"';}?>>
        </label>
